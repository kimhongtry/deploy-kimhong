          ...
          {Array.from({ length: 3 }).map((_, rowIndex) => (
            ...
            >
              {stories.map((story, index) => (
                <figure
                  key={`${rowIndex}-${index}`}
                  className="relative story-grid basis-calc shrink-0 grow-0 flex flex-col gap-y-[20px] max-w-[227px] md:max-w-[287px]"
                >
                  <picture className="border border-white rounded-[10px] md:rounded-md aspect-[375/500]">
                    <img
                      alt={story.alt}
                      loading="lazy"
                      width={375}
                      height={500}
                      decoding="async"
                      className="rounded-inherit md:rounded-md aspect-[375/500]"
                      src={story.src}
