...
//   attributes: {
//     title: string;
//     [key: string]: any; // Add fields as needed
//   };
// }
...
//       const res = await axios.get("http://62.72.46.248:1337/api/story-types");
//       console.log("Story Types Response:", res.data); // Debug log
//       const apiTypes: StoryType[] = (res.data.data || []).map((item: any) => ({
//         id: item.id,
//         name: item.attributes?.name || "Unknown", // safer access
//       }));

//       // Filter for Adventure, Classic, Animal
//       const requiredTypes: StoryType[] = [
//         { id: 2, name: "Adventure" },
//         { id: 4, name: "Classic" },
//         { id: 6, name: "Animal" },
//       ];
...
//   // Fetch stories for a given story type
//   const fetchStories = useCallback(async (typeId: number) => {
//     try {
//       let url = "http://62.72.46.248:1337/api/stories?populate=*";
//       if (typeId) {
//         url += `&filters[story_type][id][$eq]=${typeId}`;
...
//     setLoading(true);
//     try {
//       const newStoriesByType: { [key: number]: Story[] } = {};
//       for (const type of storyTypes) {
//         const stories = await fetchStories(type.id);
//         newStoriesByType[type.id] = stories;
//       }
...
//   // Render a section for each story type
//   const renderStorySection = (type: StoryType) => (
//     <div key={type.id} className="w-full max-w-6xl mb-12">
//       <h1 className="text-3xl font-bold text-gray-800 mb-6 font-BlackAndWhitePicture">
//         {type.name} Stories
//       </h1>
...
import { useEffect, useState, useCallback } from "react"; // Removed React import
import axios from "axios";
...
// Define the Story interface to match StoryCard
interface Story {
  id: number;
  title: string;
  description?: string;
...
const HomePage = () => {
  ...
  const fetchStoryTypes = useCallback(async () => {
    ...
    try {
      const res = await axios.get("http://62.72.46.248:1337/api/story-types");
      console.log("Story Types Response:", res.data);
      const apiTypes: StoryType[] = (res.data.data || []).map((item: any) => ({
        id: item.id,
  ...
  const fetchStories = useCallback(async (typeId: number) => {
    try {
      ...
      const res = await axios.get(url);
      console.log(`Stories Response for type ${typeId}:`, res.data);
      return res.data.data || [];
    } catch (error) {
      console.error(`Failed to fetch stories for type ${typeId}:`, error);
      return [];
